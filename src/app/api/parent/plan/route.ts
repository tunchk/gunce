...
import { getParentWeekPlan, PlanError } from "@/lib/plan";
...
function mapError(error: unknown) {
  ...
  console.error("parent plan api error");
...
export async function GET(request: NextRequest) {
  ...
  try {
    const data = await getParentWeekPlan(authz.session!.user.id, weekStart || undefined);
...
const FORBIDDEN = NextResponse.json(
  { error: "Veliler planı düzenleyemez." },
  { status: 403, headers: NO_STORE },
);

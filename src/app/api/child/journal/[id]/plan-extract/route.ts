...
import { getPlanExtractProvider } from "@/lib/ai";
...
function mapError(error: unknown) {
  ...
  if (error instanceof JournalError || error instanceof PlanExtractError) {
    const status =
  ...
  console.error("plan-extract api error");
  return NextResponse.json(
    { error: "Plan önerisi alınamadı." },
...
export async function GET(request: NextRequest, ctx: Ctx) {
  ...
  try {
    ...
    return NextResponse.json({ batch, openCommitments }, { headers: NO_STORE });
  } catch (error) {
...
export async function POST(request: NextRequest, ctx: Ctx) {
  ...
  const limited = await consumeRateLimit(
    `plan-extract:${authz.session!.user.id}:${ip}`,
    8,
    60_000,
  );
  if (!limited.allowed) {
    return NextResponse.json(
      { error: "Çok fazla plan önerisi denemesi. Biraz bekle." },
      { status: 429, headers: NO_STORE },
    );
  }

  const provider = getPlanExtractProvider();
  if (!provider.isConfigured()) {
    return NextResponse.json(
      {
        error:
          "Plan önerisi şu an kullanılamıyor. Planını elle ekleyebilirsin.",
        code: "NOT_CONFIGURED",
      },
  ...
  try {
    ...
    const sourceText = extractionSourceText(entryBefore);
    if (!sourceText) {
      return NextResponse.json(
    ...
    const result = await provider.extract({
      sourceText,
      diaryDate: entryBefore.diaryDate,
    ...
    if (!still) {
      return NextResponse.json(
        {
          error: "Kayıt silindiği için öneri kaydedilmedi.",
          code: "GONE",
        },
        { status: 410, headers: NO_STORE },
      );
    }
    if (still.childId !== child.id) {
    ...
    if (still.revision !== body.expectedRevision) {
      return NextResponse.json(
        {
          error:
            "Metni düzenledin; bu öneri artık güncel değil. İstersen yeniden iste.",
          code: "CONFLICT",
        },
        { status: 409, headers: NO_STORE },
      );
    ...
    return NextResponse.json({ batch, requestId }, { headers: NO_STORE });
  } catch (error) {
    return mapError(error);
  }
}
